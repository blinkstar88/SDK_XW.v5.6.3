...
{
	/* UBNT: prevent ATH_CPU_IRQ_WLAN handling on 0 status.
	???: 0 status is happening under load. This might be because
	before return handler is checking status and handles IRQ again? */
	unsigned int status = ath_reg_rd(ATH_PCIE_WMAC_INT_STATUS); 
#ifdef CONFIG_PCI
	if (likely(status & PCI_WMAC_INTR))
		do_IRQ(ATH_PCI_IRQ_DEV0);
	else
#endif
	if (unlikely(status & (WMAC_MISC_INT | WMAC_TX_INT | WMAC_RXLP_INT | WMAC_RXHP_INT)))
		do_IRQ(ATH_CPU_IRQ_WLAN);
	else if (status)
		printk(KERN_WARNING "Unindentified PCIE_WMAC INT status: %08X\n", status);
}
