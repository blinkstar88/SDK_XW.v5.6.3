...
$dyndns_servicename = cfg_get_def($cfg, "dyndns.1.servicename", $dyndnsservicename);
	...
	if ($action == "httpscertupload" && strlen($httpscertdelete) > 0) {
		...
		if ($pwdogStatus != "enabled") {
			...
			if ($pwdog_supp=="enabled") {
				cfg_set($cfg, "pwdog.command", "/bin/support /tmp/emerg /etc/persistent/emerg.supp emerg 0; reboot -f");
			} else {
		...
		if ($httpd_status != "enabled") {
			$httpd_status = "disabled";
		}
		elseif ($httpd_status == "enabled") {
		...
		cfg_set($cfg, "httpd.status", $httpd_status);
		...
		if ($rsyslog_status != "enabled") {
			$rsyslog_status = "disabled";
		}
		if ($tcpsyslog_status != "enabled") {
			$tcpsyslog_status = "disabled";
		}
		cfg_set($cfg, "syslog.status", $syslog_status);
		cfg_set($cfg, "syslog.remote.status", $rsyslog_status);
                cfg_set($cfg, "syslog.remote.tcp.status", $tcpsyslog_status);
		...
		if ($dyndnsstatus != "enabled") {
			...
			cfg_set($cfg, "dyndns.1.servicename", $dyndnsservicename);
			...
			$dyndns_servicename = $dyndnsservicename;
...
$httpd_status = cfg_get_def($cfg, "httpd.status", "enabled");
...
$tcpsyslog_status = cfg_get_def($cfg, "syslog.remote.tcp.status", $tcpsyslog_status);
