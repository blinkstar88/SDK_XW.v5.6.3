...
<?
  ...
  $autoack_disabled = 0;
  if ($polling_noack == 1 || $airsync == "enabled") {
    $autoack_disabled = 1;
  }
  if (strlen($netmode)==0) {
	$netmode = $netmode_cfg;
  ...
  $force_obey_regulatory = force_obey_regulatory($ccode);
  if ($REQUEST_METHOD == "POST") {
	if (($ccode != 511) && ($radio1_ccode_fixed == 0) && ($obey != "enabled")) {
        	if ($eirp_status != "enabled") {
                        $error_msg = dict_translate("eirp_off_obey_off|'Installer EIRP Control' cannot be disabled while 'Calculate EIRP Limit' under <a href='/link.cgi'>Wireless</a> is turned off.");
                }
        }
	...
	if ($feature_advanced_ethernet == 1) {
		if ($feature_poe_passthrough == 1) {
			...
			cfg_set($cfg, "system.poe_passthrough", $poe_pass);
		}
	...
	cfg_set($cfg, "wireless.1.l2_isolation", $client_isolation_status);
        $state = 0;
	if (strlen($mc_enhance) > 0){
        	if ($mc_enhance == "enabled") {
                        $state = 2;
                }
        }
        if ($wmode_type != 1) {
	        cfg_set($cfg, "wireless.1.mcast.enhance", $state);
        }
  ...
  $poe_pass=cfg_get_def($cfg, "system.poe_passthrough", $poe_pass);
  ...
  $chanbw = cfg_get_def($cfg, "radio.1.chanbw", $chanbw);
  ...
  $timings = get_timings($ieee_mode, $chanbw, $radio1_caps & $radio_cap_fast_clock);

  $state = "2";
  if (strlen($mc_enhance) != 0 && $mc_enhance != "enabled") {
  	$state = "0";
  }
  $mc_enhance = "enabled";
  $state = cfg_get_def($cfg, "wireless.1.mcast.enhance", $state);
  if ($state == "0") {
  	$mc_enhance = "disabled";
  }
  ...
  if ($wmode_type == 2) {
	  $qdur = cfg_get_def($cfg, "radio.1.qdur", 0);
	  if ($qdur == 0) {
		  $qdur = 40;
	  }
  }
