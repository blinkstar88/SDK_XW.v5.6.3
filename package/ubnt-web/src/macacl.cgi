...
var macacl = {
	...
	enableSave: function(enable) {
		$('#macacl_submit').prop("disabled", !enable);
	}
};
...
$(document).ready(function(){
	...
	$('#macacl').submit(function() {
		try {
			...
			alert(jsTranslate('An error has occurred') + ": " + e.message);
		}
		return false;
	});
});
		...
		<table cellspacing="0" cellpadding="0" align="center" class="popup">
			...
			<tr>
				<td colspan="3" class="centered">
					...
					<input type="submit" name="macacl_submit" id="macacl_submit" value="<? echo dict_translate("Save")>" disabled="disabled">
