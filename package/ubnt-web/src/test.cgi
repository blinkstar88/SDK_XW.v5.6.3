...
<?
				...
				$fp = fopen("/dev/mtdblock5", "r");
				fseek($fp,20);
				$dfs_lock = fgets($fp,2);
				$dfs_lock = Ord($dfs_lock);
				fclose($fp)
	...
	if (strlen($ethonly)==0) {
		...
		while ($i < $chains) {
			...
			if (($usechain == 0) || ($i == intval($usechain))){
				fputs($fp, "/sbin/factorytest setup -i $wlan_iface -e \"" + EscapeShellCmd(EscapeShellCmd($essid)) + "\" -n " + $chain_mask + $ch_sc_cmd + " -o /tmp/test-setup"+$i+".dat\n");
				fputs($fp, "sleep 1\n");
				fputs($fp, "/sbin/factorytest rssi -c $rssi_test_duration -e \"" + EscapeShellCmd(EscapeShellCmd($essid)) + "\" -i $wlan_iface -o /tmp/test-rssi"+$i+".dat\n");
				if (strlen($do)!=0) {
					fputs($fp, "sleep 1\n");
					fputs($fp, "/sbin/factorytest throughput -u \"" + EscapeShellCmd(EscapeShellCmd($file_url)) + "\" -o /tmp/test-throughput"+$i+".dat\n");
				}
			}
...
if (fileinode("/tmp/test-started") != -1) {
	...
	if (strlen($ethonly)==0) {
		...
		if (($radio1_caps & $radio_cap_dfs_lock) != 0) {
			
			if ($dfs_lock == 81){
				echo "<div class=\"msg\">\n";
				echo $dfs_msg;
				echo "</div><div class=\"success\">DFS FCC TEST SUCCESS</div>\n";
			} else {
				echo "<div class=\"errmsg\">\n";
				echo $dfs_msg;
				echo "</div><div class=\"err\">DFS FCC TEST FAILED</div>\n";
			}
