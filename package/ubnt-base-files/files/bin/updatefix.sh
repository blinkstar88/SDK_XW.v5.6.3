#!/bin/sh
[ $# -le 1 ] && echo "Usage: $0 <current_ver> <new_ver>" && exit 2
CUR_VER=$1
NEW_VER=$2
RET=0

grep -q -vE "^XM\.|^TI\.|^XW\." /usr/lib/version && exit 0
...
#New version < v5.6 then need to restore chanbw and clksel
#It is handled by current fw version fwupdate->do_downgrade_fixup if fw ver >= v5.6

#New version is newer than current one
...
#New version >= v5.5 (5<<16|5<<8)=328960
if [ ${NEW_VER} -ge 328960 ]; then
	#Current version >= v5.5.8 (5<<16|5<<8|8)=328968 and
        #New version < v5.5.8 then need to fix ME/RS country codes
	if [ ${CUR_VER} -ge 328968 -a ${NEW_VER} -lt 328968 ]; then
	        # Search for MONTENEGRO or SERBIA
                grep -qE "^radio.1.countrycode=499|^radio.1.countrycode=688" ${RUNNING_CFG}
                if [ $? -eq 0 ]; then
                	echo "Restore to SERBIA AND MONTENEGRO"
                        sed -ie "s/\.countrycode=.*/\.countrycode=891/g" ${RUNNING_CFG}
	                RET=1 #mark to write
                fi
        fi
        info_exit
...
fix_bridge() {
	#$1 is for BRIDGE status and has to be enabled|disabled
        ...
        local stp=`grep -c "^bridge\..*\.stp\.status=enabled" ${RUNNING_CFG}`
        if [ ${stp} -gt 0 ]; then
        	stp="enabled"
        else
        	stp="disabled"
...
"
[ ${NEW_VER} -eq 328709 ] || KEYS2STRIP="${KEYS2STRIP}|^radio\.1\.chanbw="
...
tr -d "\r" "\t" "\b" "\v" < ${NEW_SYSTEM_CFG} | sort | uniq > ${RUNNING_CFG}
rm -f ${NEW_SYSTEM_CFG}
echo "Done"
RET=1 #mark to write
exit ${RET}
