...
PPPOEInitDevice(void)
{
    ...
    if (!conn) {
	fatallog("Could not allocate memory for PPPoE session");
    }
...
PPPOEConnectDevice(void)
{
	...
	if (sscanf(existingSession, "%d:%x:%x:%x:%x:%x:%x",
		   ...
		   &mac[3], &mac[4], &mac[5]) != 7) {
	    fatallog("Illegal value for rp_pppoe_sess option");
	}
	...
	if (conn->discoveryState != STATE_SESSION) {
	    errorlog("Unable to complete PPPoE Discovery");
	    return -1;
	}
    }
	...
	fatallog("Failed to create PPPoE socket: %m");
    }
	...
	fatallog("Failed to connect PPPoE socket: %d %m", errno);
		...
		int accomp)
{
	...
	warnlog("Couldn't increase MRU to %d", mru);
#endif
}
...
PPPOEDisconnectDevice(void)
{
    ...
    if (connect(conn->sessionSocket, (struct sockaddr *) &sp,
		sizeof(struct sockaddr_pppox)) < 0) {
	fatallog("Failed to disconnect PPPoE socket: %d %m", errno);
	return;
    }
...
PPPoEDevnameHook(char *cmd, char **argv, int doit)
{
	...
	if (ioctl(fd, SIOCGIFINDEX, &ifr) < 0) {
		...
		if (ifr.ifr_hwaddr.sa_family != ARPHRD_ETHER) {
		    if (doit)
			errorlog("Interface %s not Ethernet", cmd);
		    r = 0;
		}
	    }
	}
...
plugin_init(void)
{
	...
	fatallog("Linux kernel does not support PPPoE -- are you running 2.4.x?");
    }

    add_options(Options);

    infolog("RP-PPPoE plugin version %s compiled against pppd %s",
	 RP_VERSION, VERSION);
}
