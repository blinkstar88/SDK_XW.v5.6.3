...
parsePacket(PPPoEPacket *packet, ParseFunc *func, void *extra)
{
    ...
    if (packet->ver != 1) {
	errorlog("Invalid PPPoE version (%d)", (int) packet->ver);
	return -1;
    }
    if (packet->type != 1) {
	errorlog("Invalid PPPoE type (%d)", (int) packet->type);
	return -1;
    }

    /* Do some sanity checks on packet */
    if (len > ETH_JUMBO_LEN - PPPOE_OVERHEAD) { /* 6-byte overhead for PPPoE header */
	errorlog("Invalid PPPoE packet length (%u)", len);
	return -1;
    }
	...
	if ((curTag - packet->payload) + tagLen + TAG_HDR_SIZE > len) {
	    errorlog("Invalid PPPoE tag length (%u)", tagLen);
	    return -1;
	}
...
clampMSS(PPPoEPacket *packet, char const *dir, int clampMss)
{
    ...
    if (csum) {
	errorlog("Bad TCP checksum %x", (unsigned int) csum);
	     ...
	     void *extra)
{
	...
	errorlog("PPP-max-payload: %.*s", (int) len, data);
	break;
    case TAG_SERVICE_NAME_ERROR:
	errorlog("PADT: Service-Name-Error: %.*s", (int) len, data);
	break;
    case TAG_AC_SYSTEM_ERROR:
	errorlog("PADT: System-Error: %.*s", (int) len, data);
	break;
    case TAG_GENERIC_ERROR:
	errorlog("PADT: Generic-Error: %.*s", (int) len, data);
	break;
    }
}
